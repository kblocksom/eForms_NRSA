...
verification_tbl <- dplyr::filter(verification, !(PARAMETER %in% c('ACTUAL_LAT_DD','ACTUAL_LON_DD',
...
samplesNotColl <- params[['userDataRMD']][['SAMPLES']][[1]]$SAMPLE_TYPE[params[['userDataRMD']][['SAMPLES']][[1]]$PARAMETER=='NOT_COLLECTED']
...
if(exists("params[['userDatRMD']][['FISH']]")){
  ...
  fishTaxa <- dplyr::filter(fishTaxa,  PARAMETER == 'NAME_COM') %>%
    dplyr::rename(`Common Name` = RESULT) %>%
    left_join(fishCounts, by= 'LINE') %>%
    dplyr::select(`Common Name`, Count) %>%
    arrange(desc(Count))
}else{
  fishTaxa <- dplyr::filter(params[['userDataRMD']][['FISHGEAR']][[1]], PARAMETER %in% c('FISH_PROTOCOL','PROTOCOL_COMMENT')) %>%
    dplyr::select(PARAMETER, RESULT) %>%
...
if(exists("params[['userDatRMD']][['FISH']]")){
  ...
  kable(fishTaxa, col.names = c('Fish Not Collected', ''), align=c('l','r')) %>%
    kable_styling(bootstrap_options = 'condensed', full_width = F, position = 'float_left') 
}
```
